...
(defn- add-joplin-deps [project]
  (-> project
      (deps/add-if-missing '[joplin.core "0.1.0"])
      ;; TODO -- leave out
      (deps/add-if-missing '[joplin.jdbc "0.1.0"])
      (deps/add-if-missing '[joplin.elasticsearch "0.1.0"])
      (deps/add-if-missing '[joplin.zookeeper "0.1.0"])))
